...
void AArch64TargetLowering::parallelCopy(MIR::Block* block) {
    ...
    for (auto& [dest, src] : m) {
        ...
        if (s == d) {
            ...
            m_instructionInfo->move(block, block->getInstructionIdx(term), s, tmp, m_dataLayout->getSize(t), classid == FPR64 || classid == FPR32);
        ...
        s = m[d].first;
        t = m[d].second;
        m_instructionInfo->move(block, block->getInstructionIdx(term), s, d, m_dataLayout->getSize(t), classid == FPR64 || classid == FPR32);
        visited.insert(d);
    }
}
